 ...
 *  - Dashboard :
 *  - Program Information Screen : Filter function to select a university(or all) and a subject area(or all) to display university programs
 *  in a scrollable area. Further information about the program is provided when the program button is clicked.
 *  - Map Screen: This screen implements the MAPS static API and Geocoding API to get convert user location to latitude and longitude,
 ...
 *  - Comparison Screen: Takes user's selected options, displaying the numerical information(Tuition, program tating, and admission average) 
 *  for the selected university programs on a generated bar graph.
 *  - User Preferences Screen :  Takes the users entered responses for each survey question.
 *  - Matching Screen : Finds and displays the top three universities  that fit the user preferences.
 *  - User Profile screen :
 *  
 * Areas of Concern:  
