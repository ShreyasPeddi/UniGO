...
public class GradesPanel extends JPanel{
	...
	JLabel courseLabels[] = new JLabel[8];
	private JButton displayAverage = new JButton("Display Average");
	private JLabel showAverage = new JLabel();
	
	public GradesPanel(){
		setBounds(1080,60,740,940);
		setLayout(null);
		setBackground(Color.LIGHT_GRAY);
		
		for(int field=0;field<courses.length;field++){
			courses[field] = new JTextField();
			courses[field].setBounds(200,20+(field*60),150,40);
			
			courseMarks[field] = new JTextField();
			courseMarks[field].setBounds(430,20+(field*60),70,40);
			
			courseLabels[field] = new JLabel("Course:" +( field + 1));
			courseLabels[field].setBounds(80,20+(field*60),100,40);
			
			add(courses[field]);
			add(courseMarks[field]);
			add(courseLabels[field]);
		}
		
		displayAverage.setBounds(430,500,120,30);
		showAverage.setBounds(270,500,150,30);
